...
const createOrder = async (productsOrderId, _id) => {
  ...
  const order = await razorpay.orders.create({
    amount: Math.round(
      Number(productsOrder.pricing.totalAmount.toFixed(2)) * 100,
    ),
    currency: "INR",
    notes: {
      useId: _id,
...
const varifyPayment = async (
  ...
  if (razorpay_signature === expectedSignature) {
    if (order.orderStatus === "paymentFailed") {
      for (let item of order.items) {
        await productModel.updateOne(
          ...
          { $inc: { "variants.$[v].stock": -Number(item.quantity) } },
          {
            arrayFilters: [
    ...
    return {
      type: "success",
      href: `/orders/orderSuccess?orderId=${order.orderId}`,
    };
  } else {
    if (order.orderStatus !== "paymentFailed") {
    ...
    return {
      ...
      href: `/orders/orderFailure?orderId=${payment.orderId}&productOrderId=${order.orderId}$reason=somthing was wroing!`,
    };
  }
};
