  ...
  pipeline.push(
    {
      $addFields: {
        finalDiscount: {
          $max: ["$product.discound", "$productDiscount", "$categoryDiscount"],
        },
      },
    },
    {
...
const checkcartItem = async (productId, variantId) => {
  const isTrue = await cartModel.find({
    productId: productId,
    variantId: variantId,
  });
  if (isTrue.length != 0) {
...
const cartSummary = (items) => {
  ...
  const totalDiscountPrice = items.reduce((total, item) => {
    total +=
      (item.product.variants.price - item.finalPrice) *
  ...
  return {
    totalPriceCartItem: totalPriceCartItem.toFixed(2),
    totalDiscountPrice: totalDiscountPrice.toFixed(2),
    tax: tax.toFixed(2),
    total: total.toFixed(2),
    offerDiscount: totalDiscountPrice.toFixed(2),
  };
};
