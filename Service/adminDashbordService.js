...
const analyseDashbordData = async (selectedType, filter) => {
        ...
        $project: {
          ...
          persentage: {
            $round: [
              {
                $multiply: [
                  { $divide: ["$products.totalQuantity", "$grandTotal"] },
                  100,
                ],
              },
              2,
        ...
        $project: {
          ...
          persentage: {
            $round: [
              {
                $multiply: [
                  { $divide: ["$categories.totalQuantity", "$grandTotal"] },
                  100,
                ],
              },
              2,
  ...
  const totalUserSginup = await User.countDocuments({
    createdAt: { $gte: startDate, $lte: endDate },
  });
  ...
  return {
    ...
    totalUserSginup: totalUserSginup || 0,
    mostSaled,
    analyseData,
    chartData,
  };
