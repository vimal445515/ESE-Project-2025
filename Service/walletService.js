...
const referralReward = async(newUserId,referralCode)=>{
    ...
    const  newUserWallet = await walletModel.findOne({userId:new mongoose.Types.ObjectId(newUserId)})
   newUserWallet.balance +=20;

    const  referedUserWallet = await walletModel.findOne({userId:new mongoose.Types.ObjectId(referedUser._id)})
