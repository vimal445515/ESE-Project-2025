...
const updateOffer = async (offerName, discount, expiryDate, offerId) => {
  await offerModel.findOneAndUpdate(
    { _id: new mongoose.Types.ObjectId(offerId) },
    { $set: { offerName, discount, expiryDate } },
  );
};
...
const calculateOffersForProducts = async (products) => {
  const result = await Promise.all(
    products.map(async (product) => {
      const categoryDiscount = await offerModel.findOne({
        isActive: true,
        expiryDate: { $gte: new Date() },
        offerType: "category",
        targetId: new mongoose.Types.ObjectId(product.categoryId),
      });
      const productDiscount = await offerModel.findOne({
        isActive: true,
        expiryDate: { $gte: new Date() },
        offerType: "product",
      ...
      if (!categoryDiscount && !productDiscount) {
        ...
        offer =
          productDiscount.discount >= categoryDiscount.discount
            ? productDiscount
            : categoryDiscount;
      }
      if (offer) {
