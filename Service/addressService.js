...
const updateAddressAsDefault = async (_id, userId) => {
  const data = await addressSchema.findOneAndUpdate(
    { userId: new mongoose.Types.ObjectId(userId), default: true },
    { $set: { default: false } },
  );
...
const deleteAddressFromDB = async (addressId, userId) => {
  if (await addressSchema.findOne({ _id: addressId, default: true })) {
    ...
    return await addressSchema.updateOne(
      { userId: userId },
      { $set: { default: true } },
    );
  }
