...
const getAllUsers = async (skip, limit, search) => {
  let pipeline = [];
  if (search)
    pipeline.push({ $match: { email: { $regex: search, $options: "i" } } });
  pipeline.push(
...
const findActiveUsers = async (skip, limit) => {
  return await User.find({ isBlocked: false, deleted: false })
    .skip(skip)
    .limit(limit);
};

const findBlockedUsers = async (skip, limit) => {
  return await User.find({ isBlocked: true, deleted: false })
    .skip(skip)
    .limit(limit);
};
...
const addCategorInDB = async (categoryName, publicId, url) => {
  ...
  return await categoryModel.insertOne({
    categoryName,
    thumbnail: imageObj,
    createdAt: Date.now(),
    isBlocked: false,
...
const updateCategory = async (
  ...
  if (publicId) {
    await categoryModel.findOneAndUpdate(
      { _id },
      { $set: { thumbnail: { url: url, publicId: publicId } } },
    );
  }
  if (name)
    await categoryModel.findOneAndUpdate(
