...
const applayCouponCodeInTotalAmount = async (products, couponCode, userId) => {
  ...
  if (oldAmount.total < coupon.minimumOrder) {
    throw new Error(
      `This coupon require a minimum amount of ${coupon.minimumOrder} INR`,
    );
  }
  if (
    await couponUseCount.findOne({
  ...
  const totalDiscountPrice =
    Number(oldAmount.totalDiscountPrice) + couponDiscount;
  ...
  products = products.map((item) => {
    ...
    let itemCouponDiscountAmount = Number((ratio * couponDiscount).toFixed(2));

    let couponAppliedFinalPrice = Number(
      (itemTotal - itemCouponDiscountAmount).toFixed(2),
    );
    return {
      ...item,
  ...
  const tax =
    ((Number(totalPriceCartItem) - Number(totalDiscountPrice)) * 18) / 100;

  const total = totalPriceCartItem - totalDiscountPrice + tax;

  return {
    totalPriceCartItem: Number(totalPriceCartItem),
    totalDiscountPrice: Number(totalDiscountPrice),
    tax: Number(tax),
    total: Number(total),
    couponDiscount: Number(couponDiscount),
    products: products,
    offerDiscount: Number(oldAmount.totalDiscountPrice),
  };
};
