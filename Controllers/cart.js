...
const addToCart = async (req, res) => {
  try {
    ...
    if (
      ...
      if (req.headers.referer.split("/").pop().trim() === "wishlist")
        return res.status(400).redirect("/wishlist");
      return res.redirect(
    ...
    const stock = await cartService.getCartSingleItem(
      req.body.productId,
      req.body.variantId,
    );
    const cartData = await cartService.getCartQuantity(
      req.body.productId,
      req.body.variantId,
    );
    ...
    try {
      if (
        await cartService.checkcartItem(req.body.productId, req.body.variantId)
      ) {
        if (req.body?.decrement) {
          if (req.body?.quantity > 1) {
            ...
            let item = await cartService.getCartSingleItem(
              req.body.productId,
              req.body.variantId,
            );
            const cartItems = await cartService.getCartItems(req.session._id);
        ...
        if (cartData.quantity + Number(quantity) > stock[0].variants.stock) {
          if (req.body?.increment) {
            req.flash(
              "error",
              `Only ${stock[0].variants.stock} items available`,
            );
            return res.json({ type: "error" });
          }
        ...
        if (
          (Number(req.body?.quantity) <= stock[0].variants.stock) &
          (req.body?.quantity <= 5)
        ) {
          if (req.body?.increment) {
      ...
      if (Number(req.body?.quantity) > stock[0].variants.stock)
        throw new Error(`Only ${stock[0].variants.stock} stock available`);
