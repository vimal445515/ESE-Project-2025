...
const loadCategoryOfferPage = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .render("Admin/categoryOfferPage", {
        categorys,
        offers,
        count,
        limit,
...
const enableDisableOffer = async (req, res) => {
  try {
    ...
    if (action) {
      await offerService.enableOffer(
        "product",
        req.body.offerId,
        req.body.productId,
      );
    ...
    res
      .status(200)
      .json({ href: `/offers/product?page=${Number(req.query.page)}` });
  } catch (error) {
    console.log(error);
...
const updateProductOffer = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ type: "success", message: "Offer updated successfully" });
  } catch (error) {
    req.flash("error", "Oops somthing was Wrong");
    res
      .status(500)
      .json({ type: "error", message: "! Oops somthing was Wrong" });
  }
};
...
const createOfferForCategory = async (req, res) => {
  try {
    ...
    await offerService.createCategoryOffer(
      offerName,
      categoryId,
      discount,
      expiryDate,
...
const enableDisableOfferForCategoryOffer = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ href: `/offers/category?page=${Number(req.query.page)}` });
  } catch (error) {
    console.log(error);
    res.status(500).json({ type: "error", message: "internal server error" });
  }
