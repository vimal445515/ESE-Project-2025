...
const createCoupon = async (req, res) => {
  try {
    ...
    await couponService.storeCouponInDB(
      discount,
      minimumOrder,
      maximumDiscount,
      expiryDate,
...
const editCoupon = async (req, res) => {
  try {
    const { couponCode, discount, minimumOrder, maximumDiscount, expiryDate } =
      req.body;
...
const removeCoupon = async (req, res) => {
  ...
  try {
    ...
    const { totalPriceCartItem, totalDiscountPrice, tax, total } =
      couponService.calculateTotalAmount(products);

    return res
      .status(200)
      .json({ totalPriceCartItem, totalDiscountPrice, tax, total });
  } catch (error) {
    return res
      .status(500)
      .json({ type: "error", message: "Internal server error" });
