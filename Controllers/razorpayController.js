...
const createOrderForRazorpay = async (req, res) => {
  try {
    ...
    res.json({
      type: "error",
      message: "Somthing was wrong faild to perform action",
    });
  }
};
...
const varifyPaymentRazorpay = async (req, res) => {
  const { razorpay_payment_id, razorpay_order_id, razorpay_signature } =
    req.body;
  try {
    const returnData = await razorpayService.varifyPayment(
      razorpay_payment_id,
      razorpay_order_id,
      razorpay_signature,
    );
    ...
    res
      ...
      .json({
        type: "ServerError",
        message: "somthing was wrong payment varification faild",
      });
  }
};
  ...
  try {
    ...
    res
      ...
      .json({
        ...
        href: `/orders/orderFailure?orderId=${orderId}&productOrderId=${order.orderId}&reason=${req.body.reson}`,
      });
  } catch (error) {
    console.log(error);
    res
...
const retryPayment = async (req, res) => {
  ...
  try {
    const order = await razorpayService.getPaymentDetails(
    ...
    if (error.message.includes("Out of Stock")) {
      ...
      res
        .status(500)
        .json({ type: "ServerError", message: "somthing was wrong" });
    }
  }
};
