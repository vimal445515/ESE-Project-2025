...
const address = async (req, res) => {
  try {
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ type: "error", message: "Connection faild" });
    }
...
const loadAddressPage = async (req, res) => {
  try {
    ...
    let allAddress = await addressService.getAllAddressForCheckout(
      req.session._id,
    );
    data = data || null;
...
const deleteAddress = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ type: "success", message: "address Deleted successfully" });
  } catch (error) {
