...
const loadSalesReportPage = async (req, res) => {
  ...
  try {
    ...
    const orders = await orderService.getOrdersForSalesReport(
      startDate,
      endDate,
      skip,
      limit,
    ...
    if (startDate || req.query.startDate === "undefined") {
      ...
      return res
        .status(200)
        .json({ salesReportData: salesReport, orders, count, limit, page });
    }

    res
      .status(200)
      .render("Admin/SalesReportPage", {
...
const downloadExcelSheet = async (req, res) => {
  try {
    ...
    const orders = await orderService.getOrdersForSalesReportDownload(
      startDate,
      endDate,
    );
    const workbook = salesReportService.generateExcelSheet(
      salesReport,
      orders,
      startDate,
      endDate,
    ...
    res.setHeader("Contont-Disposition", "attachment; filename=report.xlsx");
...
const downloadPDF = async (req, res) => {
  try {
    ...
    const report = await salesReportService.generateReportPDF(
      salesReport,
      orders,
      startDate,
      endDate,
