...
const loadCheckOutPage = async (req, res) => {
  ...
  if (path === "cart") {
    ...
    if (
      ...
      return res.redirect(
        `/productDetails/${req.query.productId}?variantId=${req.query.variantId}`,
      );
    }
    const validationData = await orderSevice.checkOrderStockAndQuantity(
      req.query.productId,
      req.query.variantId,
  ...
  const coupons = await couponService.getCouponsForCheckout(
    orderDetails,
    req.session._id,
  );
